...
REUSABLE_ARCHIVE_DIR = 'archives'
REUSABLE_META_ARCHIVE = 'reusable_meta.tar'
LINE_CHECKPOINT_ARCHIVE_PREFIX = 'line_recognizer_'
IMAGE_CACHE_ARCHIVE_PREFIX = 'image_cache_shard'
ARCHIVE_META_DIR = '_archive_meta'
...
def _reusable_meta_entries() -> list[str]:
    entries: list[Path] = [
        DATA_DIR / 'splits',
        DATA_DIR / 'results',
    ]
    charpost_dir = DATA_DIR / 'character_posterior'
    entries.extend(sorted(charpost_dir.glob('train_lm_order*_train')))
    entries.extend(sorted((charpost_dir / 'oof_ranker').glob('train_lm_order*_exclude_f*')))
    return [str(path.relative_to(VOL_DIR)) for path in entries if path.exists()]


def _line_checkpoint_entries() -> list[Path]:
    root = DATA_DIR / 'line_recognizer'
    if not root.exists():
        return []
...
def _image_cache_file_entries() -> list[str]:
    ...
    if not root.exists():
        return []
    return [str(path.relative_to(VOL_DIR)) for path in sorted(root.rglob('*')) if path.is_file()]


def _run_parallel(commands: list[list[str]], *, label: str) -> None:
    if not commands:
        return
    procs: list[tuple[int, subprocess.Popen[bytes], list[str]]] = []
    for idx, cmd in enumerate(commands, 1):
        print(f'{label} {idx}/{len(commands)} start: {" ".join(cmd)}', flush=True)
        procs.append((idx, subprocess.Popen(cmd), cmd))
    failures: list[str] = []
    for idx, proc, cmd in procs:
        rc = proc.wait()
        if rc == 0:
            print(f'{label} {idx}/{len(commands)} done', flush=True)
        else:
            failures.append(f'{idx}: rc={rc} cmd={" ".join(cmd)}')
    if failures:
...
def _build_one_archive(*, archive: Path, build_cmd, log_dir: Path, label: str) -> None:
    if archive.exists():
        ...
        return
    partial = archive.with_name(archive.name + ARCHIVE_PARTIAL_SUFFIX)
    if partial.exists():
        partial.unlink(missing_ok=True)
    log_path = log_dir / (archive.name + '.log')
    print(f'{label}: start -> {archive.name}', flush=True)
    cmd = build_cmd(partial)
    ...
    print(f'{label}: done ({size_mib:.1f} MiB) -> {archive.name}', flush=True)


def _create_reusable_archives(stage: Path, *, archive_shards: int, archive_workers: int, skip_image_cache: bool = False) -> None:
    """Build reusable tar archives preempt-safely.

    Each archive is written to <name>.tar.partial then atomically renamed to
    <name>.tar on success, so a partial file never looks complete. Archives are
    staged on the persistent volume (not a wiped tmpdir), and any archive whose
    final .tar already exists is skipped, so a preemption mid-run just means
    re-running this function: completed shards are not rebuilt.
    """
    stage.mkdir(parents=True, exist_ok=True)
    meta_dir = VOL_DIR / ARCHIVE_META_DIR
    log_dir = meta_dir / 'logs'
    log_dir.mkdir(parents=True, exist_ok=True)
    for p in stage.glob(f'*{ARCHIVE_PARTIAL_SUFFIX}'):
        p.unlink(missing_ok=True)

    specs: list[tuple[str, Path, object]] = []

    meta_entries = _reusable_meta_entries()
    if not meta_entries:
        raise RuntimeError('no reusable metadata artifacts found to archive')
    for e in meta_entries:
        print('  meta entry:', e, flush=True)
    meta_archive = stage / REUSABLE_META_ARCHIVE

    def meta_cmd(out: Path) -> list[str]:
        return ['tar', '-C', str(VOL_DIR), '-cf', str(out), *meta_entries]
    specs.append((f'meta ({len(meta_entries)} entries)', meta_archive, meta_cmd))

    line_entries = _line_checkpoint_entries()
    for path in line_entries:
        rel = str(path.relative_to(VOL_DIR))
        archive = stage / f'{LINE_CHECKPOINT_ARCHIVE_PREFIX}{path.name}.tar'

        def line_cmd(out: Path, rel=rel) -> list[str]:
            return ['tar', '-C', str(VOL_DIR), '-cf', str(out), rel]
        specs.append((f'line {path.name}', archive, line_cmd))

    image_entries = _image_cache_file_entries()
    if image_entries and not skip_image_cache:
        shard_count = min(max(int(archive_shards), 1), len(image_entries))
        print(f'image cache: {len(image_entries)} files -> {shard_count} shards', flush=True)
        shards: list[list[str]] = [[] for _ in range(shard_count)]
        for idx, entry in enumerate(image_entries):
            shards[idx % shard_count].append(entry)
        for sid, entries in enumerate(shards):
        ...
        if skip_image_cache:
            print('image cache skipped (skip_image_cache=True); no image-cache shards created', flush=True)
        else:
            print('image cache missing or empty; no image-cache shards created', flush=True)

    done_count = sum(1 for _, archive, _ in specs if archive.exists())
    pending = [(label, archive, cmd) for label, archive, cmd in specs if not archive.exists()]
    print(
        f'archives: {len(specs)} total, {done_count} already present, '
        f'{len(pending)} to build with {max(1, archive_workers)} workers',
        flush=True,
    )
    if pending:
        with ThreadPoolExecutor(max_workers=max(1, archive_workers)) as ex:
            futs = {
                ex.submit(_build_one_archive, archive=archive, build_cmd=cmd, log_dir=log_dir, label=label): label
                for label, archive, cmd in pending
            }
            for fut in as_completed(futs):
                exc = fut.exception()
                if exc is not None:
                    raise RuntimeError(f'{futs[fut]} failed: {exc}')
                try:
                    _commit_volume(f'archive {futs[fut]}')
                except Exception as commit_exc:
                    print(f'warning: vol.commit after {futs[fut]} failed: {commit_exc}', flush=True)
    total_gib = sum(p.stat().st_size for p in stage.glob('*.tar')) / (1024 ** 3)
    print(f'archive set ready: {len(specs)} files, {total_gib:.2f} GiB -> {stage}', flush=True)


def _archive_sig(paths: list[Path]) -> dict[str, object]:
    return {
        'archives': [
            {
                'path': str(path),
...
def _restore_reusable_archives() -> None:
    archive_dir = VOL_DIR / REUSABLE_ARCHIVE_DIR
    meta_archive = archive_dir / REUSABLE_META_ARCHIVE
    if not meta_archive.exists():
        return
    line_archives = sorted(archive_dir.glob(f'{LINE_CHECKPOINT_ARCHIVE_PREFIX}*.tar'))
    shard_archives = sorted(archive_dir.glob(f'{IMAGE_CACHE_ARCHIVE_PREFIX}*-of-*.tar'))
    archive_paths = [meta_archive, *line_archives, *shard_archives]
    marker = DATA_DIR / '.reusable_archives_restored.json'
    sig = _archive_sig(archive_paths)
    try:
        if marker.exists() and json.loads(marker.read_text(encoding='utf-8')) == sig:
            print('reusable archives already restored ->', archive_dir, flush=True)
            return
    except Exception:
        pass
    print('restoring reusable metadata archive ->', meta_archive, flush=True)
    _run(['tar', '-C', str(VOL_DIR), '-xf', str(meta_archive)])
    parallel_archives = [*line_archives, *shard_archives]
    if parallel_archives:
        print(f'restoring {len(parallel_archives)} reusable archive shards', flush=True)
        _run_parallel(
            [['tar', '-C', str(VOL_DIR), '-xf', str(path)] for path in parallel_archives],
            label='restore shard',
        )
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    marker.write_text(json.dumps(sig, indent=2, sort_keys=True), encoding='utf-8')
    print('restored reusable archives ->', archive_dir, flush=True)
...
def upload_artifacts(bucket_uri: str = BUCKET_URI, *, reusable_only: bool = False, archive_shards: int = 16, archive_workers: int = 8, skip_image_cache: bool = False) -> str:
    return _run_locked(
        f'upload:{"reusable" if reusable_only else "full"}',
        lambda: _upload_artifacts_unlocked(
            bucket_uri,
            reusable_only=reusable_only,
            archive_shards=archive_shards,
            archive_workers=archive_workers,
            skip_image_cache=skip_image_cache,
        ),
    )


def _upload_artifacts_unlocked(bucket_uri: str = BUCKET_URI, *, reusable_only: bool = False, archive_shards: int = 16, archive_workers: int = 8, skip_image_cache: bool = False) -> str:
    _set_env()
    vol.reload()
    _cleanup_smoke_artifacts()
    if reusable_only:
        stage = VOL_DIR / REUSABLE_ARCHIVE_DIR
        print(f'archiving reusable artifacts (archive_shards={archive_shards}, archive_workers={archive_workers}, skip_image_cache={skip_image_cache}) -> {stage}', flush=True)
        _create_reusable_archives(stage, archive_shards=archive_shards, archive_workers=archive_workers, skip_image_cache=skip_image_cache)
        target_uri = bucket_uri.rstrip('/') + '/' + REUSABLE_ARCHIVE_DIR
        print('hf sync upload ->', target_uri, flush=True)
        _run(['hf', 'sync', str(stage), target_uri])
        _commit_volume('upload reusable artifacts')
        mode = 'reusable artifacts'
        return f'uploaded {mode} to {bucket_uri}'
    stage = VOL_DIR / '_hf_bucket_stage'
    if stage.exists():
        shutil.rmtree(stage)
    (stage / 'data').mkdir(parents=True, exist_ok=True)
    print('staging full artifact tree', flush=True)
    for name in ('line_recognizer', 'splits', 'results', 'character_posterior'):
        _stage_tree(DATA_DIR / name, stage / 'data' / name)
    print('starting hf sync upload', flush=True)
    _run(['hf', 'sync', str(stage), bucket_uri])
    shutil.rmtree(stage)
...
def _stage_extracted_archives_for_upload() -> Path:
    archive_dir = VOL_DIR / REUSABLE_ARCHIVE_DIR
    stage = VOL_DIR / '_extracted_archives_upload_stage'
    if stage.exists():
        shutil.rmtree(stage)
    stage.mkdir(parents=True, exist_ok=True)

    archives = [archive_dir / REUSABLE_META_ARCHIVE]
    archives.extend(sorted(archive_dir.glob(f'{LINE_CHECKPOINT_ARCHIVE_PREFIX}*.tar')))
    archives = [path for path in archives if path.exists()]
    if not archives:
        raise FileNotFoundError(f'no reusable/line archives found under {archive_dir}')
    for archive in archives:
        print('extract for publish ->', archive, flush=True)
        _run(['tar', '-C', str(stage), '-xf', str(archive)])

    image_cache = stage / 'data' / 'character_posterior' / 'image_cache'
    if image_cache.exists():
        shutil.rmtree(image_cache)
    (stage / 'data' / 'publish_extracted_archives_manifest.json').write_text(
        json.dumps({
            'kind': 'extracted_archive_publish_manifest',
            'archives': [str(path.relative_to(VOL_DIR)) for path in archives],
            'excluded': ['data/character_posterior/image_cache'],
            'mode': 'hf sync --ignore-existing',
...
def upload_extracted_archives(
    bucket_uri: str = BUCKET_URI,
    *,
    dry_run: bool = False,
) -> str:
    return _run_locked(
        'upload-extracted-archives',
        lambda: _upload_extracted_archives_unlocked(bucket_uri=bucket_uri, dry_run=dry_run),
    )


def _upload_extracted_archives_unlocked(bucket_uri: str = BUCKET_URI, *, dry_run: bool = False) -> str:
    _set_env()
    try:
        vol.reload()
    except Exception as exc:
        print(f'warning: upload-extracted vol.reload skipped: {exc}', flush=True)
    source = _stage_extracted_archives_for_upload()
    target_uri = bucket_uri.rstrip('/') + '/data'
    cmd = ['hf', 'sync', str(source), target_uri, '--ignore-existing']
    if dry_run:
        cmd.append('--dry-run')
    print('hf sync upload ->', target_uri, '(ignore-existing)', flush=True)
    _run(cmd)
    if not dry_run:
        _commit_volume('upload extracted archives')
    return f'{"dry-run " if dry_run else ""}uploaded extracted archives to {target_uri} with --ignore-existing'
...
def run_full_pipeline(
    ...
    upload_reusable: bool = False,
    skip_image_cache: bool = False,
    archive_shards: int = 16,
    ...
    return _run_locked(
        ...
        lambda: _run_full_pipeline_unlocked(
            ...
            upload_reusable=upload_reusable,
            skip_image_cache=skip_image_cache,
            archive_shards=archive_shards,
            archive_workers=archive_workers,
        ),
...
def _run_full_pipeline_unlocked(
    ...
    Then an optional reusable-artifact upload, and a summary.
    ...
    if upload_reusable:
        print('=== full pipeline: upload-reusable ===', flush=True)
        print(upload_artifacts.remote(
            bucket_uri,
            reusable_only=True,
            archive_shards=archive_shards,
            archive_workers=archive_workers,
...
def main(
    ...
    upload_extracted_archives_only: bool = False,
    upload_extracted_dry_run: bool = False,
    archive_shards: int = 16,
    archive_workers: int = 8,
    skip_image_cache: bool = False,
    ...
    if archive_shards < 1:
        raise SystemExit('archive_shards must be >= 1')
    if archive_workers < 1:
        raise SystemExit('archive_workers must be >= 1')
    if summary:
    ...
    if upload_prod_result_only:
        ...
        return
    if upload_extracted_archives_only:
        print(upload_extracted_archives.remote(bucket_uri, dry_run=upload_extracted_dry_run))
        return
    if prod:
    ...
    if full:
        ...
        return
    if upload_reusable and not run and not score_only:
        print(upload_artifacts.remote(bucket_uri, reusable_only=True, archive_shards=archive_shards, archive_workers=archive_workers, skip_image_cache=skip_image_cache))
        return
    if upload and not run and not score_only:
        print(upload_artifacts.remote(bucket_uri, reusable_only=False, archive_shards=archive_shards, archive_workers=archive_workers))
        return
    if smoke:
        if bg:
    ...
    if not run:
        print(
            ...
            '--upload-extracted-archives-only to publish archive contents under data/ without overwrites, '
            ...
            '--upload-reusable to also upload at the end; --skip-line-training restores the old full behavior), '
            '--single-gpu-job oof0 to trial one GPU model, '
            '--score-only to run the CPU ridge/decode tail, --summary to print CERs, '
            '--upload-reusable to sync only reusable artifacts, --upload to sync all artifacts, '
            'or --status to inspect caches. Add --bg with --run/--score-only/--full to fire-and-forget '
            '(needs `modal run --detach`). --archive-shards / --archive-workers tune reusable '
            'tar sharding and parallel build workers. --skip-image-cache omits the slow '
            'image-cache shards from --upload-reusable (rebuild on the new account instead).'
        )
        return
